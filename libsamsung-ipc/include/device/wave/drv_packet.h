...
enum IpcPacketType {
	...
	WLAN_INFO_RSP		= 0x3A, //0x3A //WlanINFO		0x9, byte 0x89, byte WlanTmPowerOn, byte WlanTmPowerOff, byte WLAN_MAC[6], requested by IpcPacket with header = 57, this packet can contain MAC or smth probably
	...
	COMPASS_DATA 		= 0x4B, //0x4B //CompassData	0x9, probably reading of compass, requested by IpcPacket with header 		= 74 followed by 1 byte (equal to 1 or 2 or else (3 or 0 maybe, doesn't matter))
	DRV_STAT 			= 0x4C, //0x4C //DrvStat??		0xCC (204)
	LBS_UNK1 			= 0x49, //0x49 //LbsThing		0x6, contains databuffer generated by sub 4012DE64 GenerateLbsDataMaybe, requested by IpcPacket with header = 72, first byte != 1 and first byte != 0
	BOOT_INFO			= 0x4E, //0x4E //BootInfo		0x1, contains byte from (SharedBootInfo+0x3C), no idea what's that, requested by IpcPacket with header = 77
	PS_STOP				= 0x50, //0x50 //PS_STOP		0x1, result of proximity_sensor_detect_start, requested by IpcPacket with header = 79
	PS_START			= 0x52, //0x52 //PS_START		0x1, result of proximity_sensor_detect_stop, requested by IpcPacket with header = 81
	PS_IS_CLOSE			= 0x54, //0x54 //PS_IS_CLOSE	0x1, result of proximity_sensor_is_close, requested by IpcPacket with header = 83
	...
	TSP_UNK1			= 0x57, //TSP_related??	0x2, , requested by IpcPacket with header 		= 86 followed by probably 3 bytes
	...
	OTHERS				= 0x5A, // OTHERS O.o		0x7, string "Others", requested by IpcPacket with header 		= 89, My damn favourite one!
	FOTA_UPDATE			= 0x5C, // FOTA_Update	0x3|0x7, string "OK"|"FD"|"FC", or "FMA"|"FMB"|"FMC"|"FTA", requested by IpcPacket with header = 91
